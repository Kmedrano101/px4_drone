#include "px4_drone/offboard_position_handshake.hpp"
      ...
      declare_parameter<double>("hold_seconds", default_hold_seconds) * kLoopRateHz))
{
  ...
  // MESSAGE_VERSION=0 en ambos firmwares soportados). "" para v1.14
  // (default, FC actual), "_v1" para v1.17 (HKUST_NXT_DUAL).
  ...
  vehicle_status_sub_ = create_subscription<VehicleStatus>(
    ...
    std::bind(&OffboardPositionHandshake::vehicleStatusCallback, this, std::placeholders::_1));

  vehicle_command_ack_sub_ = create_subscription<VehicleCommandAck>(
    "/fmu/out/vehicle_command_ack", status_qos,
    std::bind(&OffboardPositionHandshake::vehicleCommandAckCallback, this, std::placeholders::_1));

  const auto period = std::chrono::duration<double>(1.0 / kLoopRateHz);
  timer_ = create_wall_timer(
    std::chrono::duration_cast<std::chrono::milliseconds>(period),
    std::bind(&OffboardPositionHandshake::onTimer, this));
...
void OffboardPositionHandshake::onTimer()
{
  switch (state_) {
    ...
    case State::kFinished:
      timer_->cancel();
      RCLCPP_INFO(get_logger(), "Test de handshake finalizado.");
      break;
  }
...
void OffboardPositionHandshake::publishOffboardControlMode()
{
...
void OffboardPositionHandshake::publishTrajectorySetpoint()
{
  // Hold fijo en el origen NED (0,0,0): no se lee vehicle_local_position
  // porque este nodo nunca comanda un despegue, asi que no importa cual
  // sea la posicion real -- solo se necesita un setpoint valido y
  // consistente para que el FC acepte y mantenga OFFBOARD.
...
void OffboardPositionHandshake::publishVehicleCommand(uint16_t command, float param1, float param2)
{
  VehicleCommand msg{};
  msg.param1 = param1;
  msg.param2 = param2;
...
void OffboardPositionHandshake::vehicleStatusCallback(const VehicleStatus::SharedPtr msg)
{
  ...
  // puede ya estar en OFFBOARD antes de que este nodo lo pida (ver el fix
  // equivalente en offboard_control.cpp / takeoff_position_hold_base.cpp).
...
void OffboardPositionHandshake::vehicleCommandAckCallback(const VehicleCommandAck::SharedPtr msg)
{
  RCLCPP_INFO(
...
std::string OffboardPositionHandshake::commandResultToString(uint8_t result)
{
...
std::string OffboardPositionHandshake::navStateToString(uint8_t nav_state)
{
  switch (nav_state) {
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<OffboardPositionHandshake>());
  rclcpp::shutdown();
  return 0;
}
